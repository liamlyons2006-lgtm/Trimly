...
export default function Insights() {
  const { subscriptions, loading } = useTrimly();
    ...
    <div className="page-wrap">
      ...
      <div className="insights-layout">
        <section className="panel panel-pad">
          ...
          <div className="insight-number" data-testid="text-insights-monthly">{formatMoney(monthly)}<small> / month</small></div>
          <span className="delta" data-testid="status-insights-delta"><ArrowDownRight size={12} /> {cancelling > 0 ? `${formatMoney(cancelling, 0)} in the maybe pile` : 'steady, for now'}</span>
      ...
      <section className="insight-callout" style={{ marginTop: 18 }} data-testid="card-insights-oh-wow">
        ...
        <p>{cancelling > 0 ? <>You have already marked <strong>{formatMoney(cancelling, 0)} per month</strong> as worth a second look. If those charges go, that is <strong>{formatMoney(cancelling * 12, 0)} back over a year</strong> — without touching anything you love.</> : <>Not every recurring payment needs trimming. The win is knowing the total, then making one small decision at a time.</>}</p>
      </section>
    </div>
  );
}